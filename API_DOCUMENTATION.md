...
#### Get Menu Data
```javascript
const response = await fetch('http://localhost:3000/api/v1/scrapers/ilcaminetto/menu');
...
#### Run Scraper
```javascript
const response = await fetch('http://localhost:3000/api/v1/scrapers/uber/scrape', {
  method: 'POST'
});
